...
from fb_search.tools.selenium_server import SeleniumServer
from fb_search.tools.extract import Extract
from fb_search.tools.connect_mysql import Save_Data

from fb_search.settings import REDIS, EPR_TIME
    ...
    def start(self, keyword):
        ...
        try:
            for num in range(1, div_num + 1):
                try:
                    xpath_rule = '//div[@role="feed"]/div/div[{}]//div[@role="article"]/div/div/div/div/div[3]/a'.format(
                        num)
...
if __name__ == '__main__':
    ...
    #             continue
    keyword_list = ['NPC', 'CPPCC', 'two sessions', "the National People's Congress",
                    "the Chinese People's Political Consulative Conference", 'delegate to NPC', 'member of CPPCC',
                    'the NPC standing Committee', 'the Standing Committee of  the CPPCC National Committee', '兩會',
                    '全國人民代表大會', '中華人民共和國全國人民代表大會', '中國人民政治協商會議', '兩會時間', '中國人民政治協商會議全國委員會常務委員會', '两会', '全国人民代表大会',
                    '中华人民共和国全国人民代表大会', '中国人民政治协商会议', '两会时间', '中国人民政治协商会议全国委员会常务委员会', '全国人民代表大会常务委员会', '全國人民代表大會常務委員會',
                    '政協委員', '政協提案']
